...
public class dfGUIManagerInspector : Editor
{
	...
	";
		...
		using( dfEditorUtil.BeginGroup( "Rendering and Behavior" ) )
		{
			...
			var currentMode = camera.isActiveAndEnabled ? 0 : 1;
			...
			if( currentMode != selectedMode )
			{
				...
				if( selectedMode == 0 )
				{
					...
					//camera.isActiveAndEnabled = true;
	...
	public void OnSceneGUI()
	{
		...
		if( SceneView.currentDrawingSceneView.camera.isActiveAndEnabled )
		{
...
//
	...
	protected void FillContextMenu( List<ContextMenuItem> menu )
	{
		...
		if( showGuides )
		{
			...
			var isOrthographic = ( scene != null ) ? scene.camera.isActiveAndEnabled : false;
			if( isOrthographic )
			{
